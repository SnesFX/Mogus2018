		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
			{
				...
				if (m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					float num19 = m_currentFontSize / m_currentFontAsset.fontInfo.PointSize * m_currentFontAsset.fontInfo.Scale;
			...
			float num44 = (m_previousLossyScaleY = base.transform.lossyScale.y);
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				if (flag15)
				{
					...
					if (!flag2 && flag16 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
					{
						flag2 = true;
				...
				if ((m_textInfo.characterInfo[j].style & FontStyles.Highlight) == FontStyles.Highlight)
				{
					...
					if (!flag3 && flag17 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
					{
						flag3 = true;
