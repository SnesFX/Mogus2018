	...
	public void Update()
	{
		...
		if (displaying == null || displaying.GetHashCode() != Text.GetHashCode() || Color != lastColor)
		{
			...
			if (!texture || !colorTexture)
			{
				if (!texture)
				{
					texture = new Texture2D(num, num3, TextureFormat.ARGB32, false);
					texture.filterMode = FilterMode.Point;
					texture.wrapMode = TextureWrapMode.Clamp;
				}
				if (!colorTexture)
				{
					colorTexture = new Texture2D(num, num3, TextureFormat.ARGB32, false);
			...
			{
				texture.Resize(num, num3, TextureFormat.ARGB32, false);
				colorTexture.Resize(num, num3, TextureFormat.ARGB32, false);
			}
