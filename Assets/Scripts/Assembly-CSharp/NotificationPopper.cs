...
public class NotificationPopper : MonoBehaviour
{
	...
	public void Update()
	{
		if (alphaTimer > 0f)
		{
			...
			if (!DestroyableSingleton<HudManager>.Instance.TaskText.isActiveAndEnabled)
			{
				...
				base.transform.localPosition = new Vector3(0f - num + 0.1f, transform.localPosition.y - height, 0f);
			}
			else
			{
				...
				base.transform.localPosition = new Vector3(0f - num + 0.1f, parent.localPosition.y - height2 - 0.2f, 0f);
			}
	...
	public void AddItem(string item)
	{
		...
		SoundManager.Instance.PlaySound(NotificationSound, false);
	}
}
