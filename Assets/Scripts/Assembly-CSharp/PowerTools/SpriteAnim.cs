...
namespace PowerTools
{
	...
	public class SpriteAnim : SpriteAnimEventHandler
	{
		...
		public string ClipName
		{
			get
			{
				return (!(m_currAnim != null)) ? string.Empty : m_currAnim.name;
			}
		}
		...
		public void Play(AnimationClip anim = null, float speed = 1f)
		{
			if (anim == null)
			{
				anim = ((!m_currAnim) ? m_defaultAnim : m_currAnim);
				if (anim == null)
				{
					return;
				}
		...
		public void SetNormalizedTime(float ratio)
		{
			if (!(m_currAnim == null))
			{
				m_animator.Play(STATE_NAME, 0, (!m_currAnim.isLooping) ? Mathf.Clamp01(ratio) : Mathf.Repeat(ratio, 1f));
			}
		}
