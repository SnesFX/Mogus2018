...
public class SampleMinigame : Minigame
{
	...
	private static string[] ProcessingStrings = new string[4] { "Take a break", "Go grab a coffee", "You dont need to wait", "go do something else" };
	...
	public override void Begin(PlayerTask task)
	{
		...
		case States.PrepareSample:
			StartCoroutine(BringPanelUp(true));
			break;
		}
	}
	...
	public void FixedUpdate()
	{
		if (State == States.Processing)
		{
			if (MyNormTask.TaskTimer <= 0f)
			{
				...
				LowerText.Text = "Select Anomaly";
				UpperText.Text = string.Empty;
				int num = Tubes.RandomIdx();
				State |= (States)(byte)num;
				Tubes[num].color = Color.red;
				LowerButton.color = Color.white;
				StartCoroutine(BringPanelUp(false));
			}
			else
			{
		...
		else if ((State & States.Selection) != 0)
		{
			float num2 = Mathf.Cos(Time.time * 1.5f) - 0.2f;
			Color color = new Color(num2, 1f, num2, 1f);
		...
		{
			float num3 = Mathf.Cos(Time.time * 1.5f) - 0.2f;
			Color color2 = new Color(num3, 1f, num3, 1f);
	...
	public IEnumerator BringPanelUp(bool isBeginning)
	{
		...
		{
			SoundManager.Instance.PlaySound(PanelMoveSound, false);
		}
	...
	private IEnumerator DropTube(int id)
	{
		if (Constants.ShouldPlaySfx())
		{
			SoundManager.Instance.PlaySound(dropSounds.Get(), false);
		}
	...
	{
		if ((State & States.Selection) != 0)
		{
			int correctTube = (int)(State & (States)15);
			State = States.PrepareSample;
			for (int i = 0; i < Buttons.Length; i++)
			{
				Buttons[i].color = Color.white;
			}
			StartCoroutine(CoSelectTube(correctTube, tubeId));
		}
	}
	...
	private IEnumerator CoSelectTube(int correctTube, int selectedTube)
	{
		...
		else
		{
			if (Constants.ShouldPlaySfx())
			{
				SoundManager.Instance.PlaySound(ButtonSound, false, 0.6f);
			}
		...
		int remaining = MyNormTask.MaxStep - MyNormTask.taskStep;
		if (remaining == 0)
		{
		...
		if (!MyNormTask.IsComplete)
		{
			yield return BringPanelUp(true);
		}
	}
	...
	public void NextStep()
	{
		if (State == States.AwaitingStart)
		{
			...
			if (Constants.ShouldPlaySfx())
			{
				AudioSource audioSource = SoundManager.Instance.PlaySound(ButtonSound, false);
	...
	private IEnumerator DropLiquid()
	{
		...
		for (int step = -2; step < 2; step++)
		{
			...
			yield return dropWait;
			int tubeId = step + 3;
			yield return DropTube(tubeId);
		}
		for (float j = 0f; j < 0.15f; j += Time.deltaTime)
		{
			pos.x = dropperX.Lerp(1f - j / 0.15f);
			Dropper.transform.localPosition = pos;
			yield return wait;
		}
		pos.x = dropperX.min;
