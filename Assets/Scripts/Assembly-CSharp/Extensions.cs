...
public static class Extensions
{
	...
	public static int ToInteger(this Color c, bool alpha)
	{
		if (alpha)
		{
			return ((byte)(c.r * 256f) << 24) | ((byte)(c.g * 256f) << 16) | ((byte)(c.b * 256f) << 8) | ((byte)(c.a * 256f) << 0);
		}
		return ((byte)(c.r * 256f) << 16) | ((byte)(c.g * 256f) << 8) | ((byte)(c.b * 256f) << 0);
	}
	...
	public static void Shuffle<T>(this IList<T> self)
	{
		...
		{
			T value = self[i];
			int index = (int)(UnityEngine.Random.value * (float)self.Count);
			self[i] = self[index];
			self[index] = value;
		}
	}
	...
	{
		T val = self.First();
		decimal num = comparer(val);
		for (int i = 0; i < self.Count; i++)
		{
			T val2 = self[i];
			decimal num2 = comparer(val2);
			if (num < num2 || (num == num2 && UnityEngine.Random.value > 0.5f))
			{
				num = num2;
				val = val2;
