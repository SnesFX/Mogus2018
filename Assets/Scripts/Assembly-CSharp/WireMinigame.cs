...
public class WireMinigame : Minigame
{
	...
	public void Update()
	{
		...
		for (int i = 0; i < LeftNodes.Length; i++)
		{
			...
			{
				Vector2 vector2 = myController.DragPosition;
				WireNode wireNode = CheckRightSide(vector2);
				if ((bool)wireNode)
				{
					vector2 = wireNode.transform.position;
					ActualWires[wire.WireId] = wireNode.WireId;
				}
				else
				{
					vector2 -= wire.BaseWorldPos.normalized * 0.05f;
					ActualWires[wire.WireId] = -1;
				}
				wire.ResetLine(vector2);
				break;
			}
			case DragState.Released:
				...
				{
					SoundManager.Instance.PlaySound(WireSounds.Random(), false);
				}
				CheckTask();
				break;
			}
