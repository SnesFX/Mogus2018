...
public class CourseMinigame : Minigame
{
	...
	public void FixedUpdate()
	{
		...
		case DragState.Dragging:
			...
			{
				Vector2 vector5 = (Vector2)PathPoints[num2 + 1] - vector;
				Vector2 vector6 = new Vector2(1f, vector5.y / vector5.x);
				Vector2 vector7 = (Vector2)base.transform.InverseTransformPoint(myController.DragPosition) - vector;
				if (vector7.x > 0f)
				{
					...
					if (Mathf.Abs(vector8.y - vector7.y) < 0.5f)
					{
						...
						Ship.transform.localPosition = localPosition3;
						Ship.transform.eulerAngles = new Vector3(0f, 0f, Vector2.up.AngleSigned(vector5));
					}
					else
					{
						myController.Reset();
		...
		case DragState.NoTouch:
			if (num < (float)(PathPoints.Length - 1))
			{
				...
				Vector2 vector4 = vector3 * (num - (float)num2);
				Vector3 localPosition = vector4 + vector;
				localPosition.z = -1f;
		...
		if ((int)num > num2 && (bool)Stars[num2 + 1])
		{
			...
			if (num2 == PathPoints.Length - 2)
			{
				...
				{
					AudioSource audioSource = SoundManager.Instance.PlaySound(SetCourseLastSound, false);
			...
			{
				AudioSource audioSource2 = SoundManager.Instance.PlaySound(SetCourseSound, false);
