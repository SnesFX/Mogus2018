	...
	public IEnumerator DoClick(float duration)
	{
		for (float time = 0f; time < duration; time += Time.deltaTime)
		{
			...
			if (t < 0.4f)
			{
				...
				if (num < 0f)
				{
					float fingerAngle = Mathf.Lerp(liftedAngle, liftedAngle * 2f, 1f + Mathf.Abs(num));
					SetFingerAngle(fingerAngle);
				}
				else
				{
					float fingerAngle2 = Mathf.Lerp(liftedAngle * 2f, 0f, num);
			...
			else
			{
				...
				Click.enabled = false;
				float fingerAngle3 = Mathf.Lerp(0f, liftedAngle, t2);
